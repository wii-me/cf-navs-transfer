...
> [!NOTE]
> **GitHub About 简介推荐（可直接复制填入仓库 Description）**：
> - **精炼版**：`基于 CF-Navs 增强的现代化起始页导航系统，保留原版全部优势的同时，新增跨设备跨平台便笺随手记与大文件极速传输功能。全边缘 Serverless 驱动。`
> - **丰富版**：`基于 CF-Navs 深度增强的云原生个人导航与起始页。在完整保留原版两级分类、22套主题、私密书签与 Serverless 零成本特性的基础上，新增跨设备跨平台轻量便笺随手记与 50MB 大文件极速传输功能（支持剪贴板即贴即传、图片灯箱预览与到期自毁）。依托 Cloudflare Workers + D1 + KV + R2 全边缘驱动。`
